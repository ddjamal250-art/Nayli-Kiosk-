...
class MasterCatalogService {
  ...
  Future<void> init() async {
    try {
      // The massive embedded JSON files have been removed to reduce app size.
      // Products can now be imported manually by the merchant via CSV/JSON.
      
      // We only load the hardcoded seed categories/items if needed
      for (final item in MasterCatalogSeed.items) {
        _barcodeMap[item.barcode.trim()] = item;
        _allItems.add(item);
      }
      ...
      debugPrint('MasterCatalog init error: ');
    }
  }
